...
| `status`             | `string`                 | no       | `'draft'` | Backed by `VariantStatus` enum            |
| `activated_at`       | `timestampTz`            | yes      | —         | When variant became active                |
| `deactivated_at`     | `timestampTz`            | yes      | —         | When variant was deactivated/paused       |
| `retired_at`         | `timestampTz`            | yes      | —         | When variant was permanently retired      |
| `archived_at`        | `timestampTz`            | yes      | —         | When variant was archived                 |
| `settings`           | `jsonb`                  | yes      | —         |                                            |
| `created_at`         | `timestampTz`            | no       | —         |                                            |
| `updated_at`         | `timestampTz`            | no       | —         |                                            |
...
`is_active` has been replaced by `status` (VariantStatus enum) with lifecycle timestamps (activated_at, deactivated_at, retired_at, archived_at).

### P3 — ~~HIGH: Variant has no lifecycle status~~

Variant now has a `status` column backed by `VariantStatus` enum.

### P4 — ~~MEDIUM: No `archived_at` timestamp on experiments~~

`archived_at`, `paused_at`, and `concluded_at` are included in the experiments create migration.

### P5 — ~~MEDIUM: Missing lifecycle timestamps on experiments~~

`paused_at` and `concluded_at` are included in the experiments create migration.

### P6 — ~~LOW: No lifecycle timestamps on variants~~

`activated_at`, `deactivated_at`, `retired_at`, and `archived_at` are included in the variants create migration.
